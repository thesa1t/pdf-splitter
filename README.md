...
- **Страниц на документ** — по сколько страниц подряд склеивать в один файл. Regex ищется в объединённом тексте блока, результат — один PDF на весь блок. По умолчанию 1.
- **Имя файла** и **Подпапка** — шаблоны вида `{name|cap} {surname|cap}.pdf` или `пп{amount}`. Слэш в подпапке = вложенные папки.
- **Фильтры**: `|cap` (capitalize), `|upper`, `|lower`, `|title`. Подстановки: `{page}` — первая страница блока, `{page_end}` — последняя, `{pages}` — диапазон (`5-7` или `5`, если блок из одной страницы).
